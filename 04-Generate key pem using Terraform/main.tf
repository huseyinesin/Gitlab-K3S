terraform {
  required_providers {
    aws = {
      source  = "hashicorp/aws"
      version = ">= 3.59.0"
    }
  }
}

provider "aws" {
  region = "us-east-1"
}

# Before the ssh connection change the key file permission with this command 
# chmod 400 key_name
resource "aws_key_pair" "huseyin_key" {
  key_name   = "huseyin_key"
  public_key = tls_private_key.rsa.public_key_openssh
}

resource "tls_private_key" "rsa" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

resource "local_file" "huseyin_key" {
    content  = tls_private_key.rsa.private_key_pem
    filename = "${aws_key_pair.huseyin_key.key_name}.pem"
}

resource "aws_instance" "master" {
  ami                    = "ami-08d4ac5b634553e16"
  key_name               = "huseyin_key"
  vpc_security_group_ids = [aws_security_group.k3s_server.id]
  instance_type          = "t3a.medium"
  user_data = base64encode(templatefile("${path.module}/server-userdata.tmpl", {
    token = random_password.k3s_cluster_secret.result
  })) # token = shared secret used to join a server or agent to a cluster



  tags = {
    Name = "k3sServer"
  }
}



resource "aws_instance" "worker" {
  ami                    = "ami-08d4ac5b634553e16"
  key_name               = "huseyin_key"
  vpc_security_group_ids = [aws_security_group.k3s_agent.id]
  instance_type          = "t3a.medium"
  user_data = base64encode(templatefile("${path.module}/agent-userdata.tmpl", {
    host  = aws_instance.master.private_ip,
    token = random_password.k3s_cluster_secret.result
  }))
  depends_on = [aws_instance.master]
  tags = {
    Name = "k3sWorker"
  }
}

